...
from segformer_models.segformer.backbones import mit_b0, mit_b1, mit_b2, mit_b3, mit_b4, mit_b5, MixTransformer
from segformer_models.segformer.heads import SegFormerHead
...
class SegFormer(nn.Module):
    def __init__(self, backbone: MixTransformer, decode_head: SegFormerHead, context_window=0, trained_with_deltas=True):
        super().__init__()
        self.backbone = backbone
        self.trained_with_deltas = trained_with_deltas
        if self.trained_with_deltas:
            self.delta_backbone = copy.deepcopy(backbone)
    ...
    def forward(self, x):
        ...
        if self.trained_with_deltas:
            for i in range(self.context_window*2 + 1):
                features.append(self.delta_backbone(x[:, i*3:i*3+3, ...]))
            features[self.context_window] = self.backbone(x[:, self.context_window*3:self.context_window*3+3, ...])
        else:
            for i in range(self.context_window*2 + 1):
                features.append(self.backbone(x[:, i*3:i*3+3, ...]))

        x = [torch.cat(tensors, dim=1) for tensors in zip(*features)]
        x = self.decode_head(x)
        x = interpolate(x, size=image_hw, mode='bilinear', align_corners=self.align_corners)
        return x


def create_segformer_b0(num_classes, context_window=0):
    n = context_window*2 + 1
    backbone = mit_b0()
    head = SegFormerHead(
        in_channels=(32*n, 64*n, 160*n, 256*n),
        dropout_p=0.1,
        num_classes=num_classes,
        align_corners=False,
        embed_dim=256,
    )
    return SegFormer(backbone, head, context_window)


def create_segformer_b1(num_classes, context_window=0):
    n = context_window*2 + 1
    backbone = mit_b1()
    head = SegFormerHead(
        in_channels=(64*n, 128*n, 320*n, 512*n),
        dropout_p=0.1,
        num_classes=num_classes,
        align_corners=False,
        embed_dim=256,
...
def create_segformer_b2(num_classes, context_window=0):
    n = context_window*2 + 1
    backbone = mit_b2()
    head = SegFormerHead(
        in_channels=(64*n, 128*n, 320*n, 512*n),
...
def create_segformer_b3(num_classes, context_window=0):
    n = context_window*2 + 1
    backbone = mit_b3()
    head = SegFormerHead(
        in_channels=(64*n, 128*n, 320*n, 512*n),
...
def create_segformer_b4(num_classes, context_window=0):
    n = context_window*2 + 1
    backbone = mit_b4()
    head = SegFormerHead(
        in_channels=(64*n, 128*n, 320*n, 512*n),
...
def create_segformer_b5(num_classes, context_window=0):
    n = context_window*2 + 1
    backbone = mit_b5()
    head = SegFormerHead(
        in_channels=(64*n, 128*n, 320*n, 512*n),
...
def segformer_b0(pretrained=False, progress=True, num_classes=150, context_window=0):
    ...
    """
    model = create_segformer_b0(num_classes=num_classes, context_window=context_window)
    if pretrained:
...
def segformer_b1(pretrained=False, progress=True, num_classes=150, context_window=0):
    ...
    Args:
        ...
        num_classes: Number of output classes;.
    """
    model = create_segformer_b1(num_classes=num_classes, context_window=context_window)
    if pretrained:
...
def segformer_b2(pretrained=False, progress=True, num_classes=150, context_window=0):
    ...
    Args:
        ...
        num_classes: Number of output classes;.
    """
    model = create_segformer_b2(num_classes=num_classes, context_window=context_window)
    if pretrained:
...
def segformer_b3(pretrained=False, progress=True, num_classes=150, context_window=0):
    ...
    Args:
        ...
        num_classes: Number of output classes;.
    """
    model = create_segformer_b3(num_classes=num_classes, context_window=context_window)
    if pretrained:
...
def segformer_b4(pretrained=False, progress=True, num_classes=150, context_window=0):
    ...
    Args:
        ...
        num_classes: Number of output classes;.
    """
    model = create_segformer_b4(num_classes=num_classes, context_window=context_window)
    if pretrained:
...
def segformer_b5(pretrained=False, progress=True, num_classes=150, context_window=0):
    ...
    Args:
        ...
        num_classes: Number of output classes;.
    """
    model = create_segformer_b5(num_classes=num_classes, context_window=context_window)
    if pretrained:
